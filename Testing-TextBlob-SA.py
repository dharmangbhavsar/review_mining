...
"""
#documents = ["This is so good", "This is so bad", "This is okay"]
#for key in documents:
 #   sentiments = TextBlob(key)
  #  s= sentiments.sentiment.polarity
   # print(s)
df = pd.read_csv("data.csv", delimiter = '\t', error_bad_lines = False)
#print(df['Sentence'])
#indexer = ["User", "Tweet", "Latitude", "Longitude", "Sentiment", "ID"]
s=[]
for key in df['Sentence']:
    #print(key)    
    sentiments = TextBlob(key)
    setup = sentiments.sentiment.polarity
    
    #print(setup)
    #df['sentiment'] = str(float(setup))
    if(float(setup) > float(0)):
        s.append(int(1))
        print(setup, "     here")
    else:
        print(setup,"      outhere")
